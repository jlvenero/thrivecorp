...
import AdminAprovarAcademias from './index'; // Ajuste o caminho se necessário
...
// 3. Mock do ConfirmationDialog
// Isso simplifica o teste, evitando lidar com Portals do MUI.
// Renderizamos apenas um botão "CONFIRMAR_MOCK" quando o modal estiver aberto.
...
describe('AdminAprovarAcademias Component', () => {
  ...
  it('deve buscar e renderizar a lista de academias corretamente', async () => {
    ...
    // Verifica chamada da API com endpoint correto (/api/gyms/all)
  ...
  it('deve filtrar por nome ou endereço', async () => {
    ...
    // Teste filtro por endereço
    fireEvent.change(searchInput, { target: { value: 'Av. Central' } });

    expect(screen.getByText('Studio Zen')).toBeInTheDocument(); // Endereço bate
    expect(screen.queryByText('Academia Power')).not.toBeInTheDocument(); // Não bate
  ...
  it('deve abrir modal e aprovar uma academia pendente', async () => {
    ...
    axios.put.mockResolvedValueOnce({}); // Sucesso no approve

    render(<AdminAprovarAcademias />);
    await waitFor(() => screen.getByText('Academia Power'));

    // Seleciona a linha da Academia Power
    const row = screen.getByText('Academia Power').closest('tr');
    
    // Clica no botão de aprovar (CheckCircleOutlineIcon)
    ...
    // Verifica reload da lista
    expect(axios.get).toHaveBeenCalledTimes(2);
  });

  it('deve abrir modal e reprovar uma academia pendente', async () => {
    axios.get.mockResolvedValue({ data: mockGyms });
    axios.delete.mockResolvedValueOnce({}); // Sucesso no reprove
    ...
    // Clica no botão de reprovar (HighlightOffIcon)
  ...
  it('não deve exibir botões de ação para academias já aprovadas (active)', async () => {
    ...
    // Academias ativas não têm botões de aprovar/reprovar neste código
